                ...
                <div class="panel panel-primary filterable" style="overflow:auto;">
                    ...
                    <div class="panel-body">
                        <table class="table table-striped table-bordered" id="table1">
                            ...
                            <tbody>
                                @foreach($ustored as $key =>$us)
                                <tr>
                                    <td>{{ $us->utility_name}}</td>                          
                                    <td>{{ $us->utility_description}}</td>
                                    <td>{{ $us->utility_issued_date}}</td>                          
                                    <td>{{ $us->utility_active}}</td>
                                    <td>
                                        <div class="col-sm-1">
                                            <a href="#">
                                                <i class="glyphicon glyphicon-download-alt" data-toggle="modal" data-href="#full-width" href="#responsive_{{ $us->utility_id }}" data-c="#f56954" data-hc="#f56954" title="Store"></i>
                                            </a>

                                            <div class="modal fade in" id="responsive_{{ $us->utility_id }}" tabindex="-1" role="dialog" aria-hidden="false" style="display:none;">
                                            <div class="modal-dialog modal-md">
                                                <div class="modal-content">
                                                        <form method="POST" class="form-horizontal" action="{{ url('ManageUtility') }}">
                                                         {{ csrf_field() }}

                                                            <div class="modal-header">
                                                                <h5><b>Utility Name:</b> {{ $us->utility_name}}</h5>
                                                            </div>
                                                            ...
                                                            <div class="modal-footer">
                                                                <div style="float: right" >
                                                                    ...
                                                                    <a type="submit" href="{{ route('ManageUtilityStored_uti', ['id' =>$us->utility_id]) }}" class="btn btn-primary" value="Submit" title="Submit">Store</a>
                                                            </div>
                                                            </div>
